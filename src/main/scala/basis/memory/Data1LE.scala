...
final class Data1LE(final val words: Array[Byte]) extends AnyVal with DataLE {
  ...
  final override def copy(size: Long = this.size): Data1LE = {
...
object Data1LE extends Allocator with (Long => Data1LE) {
  ...
  def unapply(data: Data1LE): Some[Array[Byte]] = Some(data.words)
