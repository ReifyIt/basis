...
/** An asbtract 2 by 2 matrix space over a field.
  ...
  */
trait F2x2[V <: F2[S] with Singleton, S <: Field with Singleton] extends Ring with FMxN[V, V, S] {
  trait Element extends Any with super[Ring].Element with super[FMxN].Element {
    override protected def Matrix: F2x2.this.type = F2x2.this
    ...
    override def trace(implicit isSquare: V =:= V): Scalar = _1_1 + _2_2
  }
...
object F2x2 {
  /** Returns a 2 by 2 matrix space over the given field. */
  def apply(Scalar: Field)(Vector: F2[Scalar.type]): F2x2[Vector.type, Scalar.type] =
    new Space[Vector.type, Scalar.type](Scalar)(Vector)
  
  /** A generic 2 by 2 matrix space over a field.
    * 
    * @tparam V   The space of rows and columns.
    * @tparam S   The set of scalars.
    */
  private final class Space[V <: F2[S] with Singleton, S <: Field with Singleton]
