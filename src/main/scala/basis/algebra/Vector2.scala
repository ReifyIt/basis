...
object Vector2 {
  ...
  implicit def dotProduct[Scalar <: Ring[Scalar]] =
    genericDotProduct.asInstanceOf[InnerProduct[Vector2[Scalar], Scalar]]
  
  private val genericDotProduct = InnerProduct[Vector2[Nothing], Ring[Nothing]] {
    (u, v) => (u ⋅ v).asInstanceOf[Ring[Nothing]]
  }
  
  /** Returns the euclidean norm typeclass for a kind of 2-dimensional vector. */
  implicit def euclideanNorm[Scalar <: CompleteField[Scalar]] =
    genericEuclideanNorm.asInstanceOf[Norm[Vector2[Scalar], Scalar]]
  
  private val genericEuclideanNorm = Norm[Vector2[Nothing], CompleteField[Nothing]] {
