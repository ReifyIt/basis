...
/** An abstract ''M'' by ''N'' real matrix space.
  ...
  */
trait RMxN[V <: RN with Singleton, W <: RN with Singleton] extends FMxN[V, W, Real.type] {
  trait Element extends Any with super.Element {
    ...
    override def det(implicit isSquare: V =:= W): Real = sys.error("not implemented")
    
    override def trace(implicit isSquare: V =:= W): Real = {
      assume(M == N)
      val dim = M * N
      var s = 0.0
      var k = 0
  ...
  override def compose[U <: FN[Real.type] with Singleton]
      (that: FMxN[U, V, Real.type]): FMxN[U, W, Real.type] = {
    if (that.isInstanceOf[RMxN[_, V]])
      compose(that.asInstanceOf[RMxN[U with RN, V]]).asInstanceOf[FMxN[U, W, Real.type]]
  ...
  override def product[U <: FN[Real.type] with Singleton](
      matrixA: A#Element forSome { type A <: FMxN[U, W, Real.type] },
      matrixB: B#Element forSome { type B <: FMxN[V, U, Real.type] }): Matrix = {
