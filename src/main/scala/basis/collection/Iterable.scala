...
trait Iterable[+A] extends Any with Traversable[A] {
  ...
  override def view: IterableView[A] = new View
  
  private final class View extends AbstractIterableView[A] {
    override def iterator: Iterated[A] = Iterable.this.iterator
  }
}
