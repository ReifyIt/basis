...
/** A 4-dimensional real vector space.
  ...
  * @author Chris Sachs
  */
object R4 extends F4[Binary64.type] with RN {
  final class Element(val x: Scalar, val y: Scalar, val z: Scalar, val w: Scalar)
    ...
    override def norm: Scalar = (x * x + y * y + z * z + w * w).sqrt
