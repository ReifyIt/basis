...
object Mold extends CollectionMolds {
  ...
  private final class ByteMold(override val identity: Byte) extends Mold[Byte] {
    override def form(variant: Variant)(value: Byte): variant.AnyForm = variant.NumberForm(value)
    ...
    override def norm(variant: Variant)(form: variant.AnyForm): variant.AnyForm = {
      if (form.isNumberForm) form
        ...
        try variant.NumberForm(java.lang.Byte.parseByte(form.asStringForm.toUString.toString))
  ...
  private final class ShortMold(override val identity: Short) extends Mold[Short] {
    override def form(variant: Variant)(value: Short): variant.AnyForm = variant.NumberForm(value)
    ...
    override def norm(variant: Variant)(form: variant.AnyForm): variant.AnyForm = {
      if (form.isNumberForm) form
        ...
        try variant.NumberForm(java.lang.Short.parseShort(form.asStringForm.toUString.toString))
        catch { case _: NumberFormatException => form }
      else form
    }
  ...
  def Date(identity: java.util.Date): Mold[java.util.Date] = new DateMold(identity)

  implicit lazy val Date: Mold[java.util.Date] = new DateMold(new java.util.Date(0L))

  private final class DateMold(override val identity: java.util.Date) extends Mold[java.util.Date] {
    override def form(variant: Variant)(date: java.util.Date): variant.AnyForm = variant.DateForm(date.getTime)

    override def cast(variant: Variant)(form: variant.AnyForm): Maybe[java.util.Date] = {
      if (form.isDateForm) Bind(new java.util.Date(form.asDateForm.millis))
      else if (form.isNumberForm) Bind(new java.util.Date(form.asNumberForm.toLong))
  ...
  private[form] final class CollectionMold[CC[X] <: Traverser[X], A](implicit CC: generic.CollectionFactory[CC], A: Mold[A]) extends Mold[CC[A]] {
    ...
    override def toString: String = "Mold"+"."+"Collection"+"("+ CC +", "+ A +")"
  }
...
private[form] class CollectionMolds {
  implicit def Collection[CC[X] <: Traverser[X], A](implicit CC: generic.CollectionFactory[CC], A: Mold[A]): Mold[CC[A]] = new Mold.CollectionMold[CC, A]

//implicit def Map[CC[X, Y] <: Map[X, Y], T](implicit CC: generic.MapFactory[CC], T: Mold[T]): Mold[CC[String, T]] = new Mold.MapMold[CC, T]
}
