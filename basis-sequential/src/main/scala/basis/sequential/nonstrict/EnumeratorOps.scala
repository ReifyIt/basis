...
final class EnumeratorOps[+A](val these: Enumerator[A]) extends AnyVal {
  ...
  def collect[B](q: PartialFunction[A, B]): Enumerator[B] =
    new EnumeratorOps.Collect(these, q)
  ...
  def flatMap[B](f: A => Enumerator[B]): Enumerator[B] =
    new EnumeratorOps.FlatMap(these, f)
  ...
  def filter(p: A => Boolean): Enumerator[A] =
    new EnumeratorOps.Filter(these, p)
  ...
  def dropWhile(p: A => Boolean): Enumerator[A] =
    new EnumeratorOps.DropWhile(these, p)
  ...
  def takeWhile(p: A => Boolean): Enumerator[A] =
    new EnumeratorOps.TakeWhile(these, p)
  ...
  def drop(lower: Int): Enumerator[A] =
    new EnumeratorOps.Drop(these, lower)
  ...
  def take(upper: Int): Enumerator[A] =
    new EnumeratorOps.Take(these, upper)
  ...
  def slice(lower: Int, upper: Int): Enumerator[A] =
    new EnumeratorOps.Slice(these, lower, upper)
  ...
  def ++ [B >: A](those: Enumerator[B]): Enumerator[B] =
    new EnumeratorOps.++(these, those)
}

private[nonstrict] object EnumeratorOps {
