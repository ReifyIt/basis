...
/** General enumerator operations.
  ...
  */
final class EnumeratorOps[+A](val these: Enumerator[A]) extends AnyVal {
  ...
  def fold[B >: A](z: B)(op: (B, B) => B): B = {
    val f = new EnumeratorOps.FoldLeft(z)(op)
    traverse(these)(f)
    f.state
  }
  ...
  def reduce[B >: A](op: (B, B) => B): B = {
    val f = new EnumeratorOps.ReduceLeft(op)
    traverse(these)(f)
  ...
  def reduceLeft[B >: A](op: (B, A) => B): B = {
    val f = new EnumeratorOps.ReduceLeft(op.asInstanceOf[(B, B) => B])
  //val f = new EnumeratorOps.ReduceLeft(op) // SI-6482
    traverse(these)(f)
    if (f.isDefined) f.state
    else throw new UnsupportedOperationException
  }
  ...
  def find(p: A => Boolean): Option[A] = {
    val f = new EnumeratorOps.Find(p)
    label(traverse(these)(f))
    f.state
  }
  ...
  def forall(p: A => Boolean): Boolean = {
    val f = new EnumeratorOps.Forall(p)
    label(traverse(these)(f))
    f.state
  }
  ...
  def exists(p: A => Boolean): Boolean = {
    val f = new EnumeratorOps.Exists(p)
    label(traverse(these)(f))
    f.state
  }
  ...
  def count(p: A => Boolean): Int = {
    val f = new EnumeratorOps.Count(p)
    traverse(these)(f)
    f.state
  }
  ...
  def choose[B](q: PartialFunction[A, B]): Option[B] = {
    val f = new EnumeratorOps.Choose(q)
    label(traverse(these)(f))
    f.state
  }
}

private[general] object EnumeratorOps {
  ...
  final class Choose[-A, +B](q: scala.PartialFunction[A, B]) extends AbstractFunction1[A, Unit] {
    ...
    override def apply(x: A): Unit = if (q.isDefinedAt(x)) { r = Some(q(x)); label.break() }
