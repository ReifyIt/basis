...
private[sequential] object NonStrictSetOps {
  ...
  class Filter[+A](
    ...
    extends NonStrictContainerOps.Filter[A](base, p) with Set[A] {
  ...
  class DropWhile[+A](
      protected[this] override val base: Set[A],
      protected[this] override val p: A => Boolean)
    extends NonStrictContainerOps.DropWhile[A](base, p) with Set[A]
  
  class TakeWhile[+A](
      protected[this] override val base: Set[A],
      protected[this] override val p: A => Boolean)
    extends NonStrictContainerOps.TakeWhile[A](base, p) with Set[A]
  
  class Drop[+A](
      protected[this] override val base: Set[A],
      protected[this] override val lower: Int)
    extends NonStrictContainerOps.Drop[A](base, lower) with Set[A]
  
  class Take[+A](
      protected[this] override val base: Set[A],
      protected[this] override val upper: Int)
    extends NonStrictContainerOps.Take[A](base, upper) with Set[A]
  
  class Slice[+A](
      protected[this] override val base: Set[A],
      protected[this] override val lower: Int,
      protected[this] override val upper: Int)
    extends NonStrictContainerOps.Slice[A](base, lower, upper) with Set[A]
  
  class ++[+A](
      protected[this] override val xs: Set[A],
      protected[this] override val ys: Set[A])
    extends NonStrictContainerOps.++[A](xs, ys) with Set[A] {
