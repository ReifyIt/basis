...
final class UString(val __ : String) extends AnyVal with Equals with Family[UString] with UTF16 {
  ...
  override def utf8Length: Int = new sequential.GeneralIteratorOps(utf8Iterator).length
  ...
  override def utf32Length: Int = new sequential.GeneralIteratorOps(utf32Iterator).length
