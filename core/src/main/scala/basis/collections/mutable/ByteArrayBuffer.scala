...
private[collections] class ByteArrayBuffer private (
  ...
  override def append(elem: Byte): Unit = {
    var array = buffer
  ...
  override def appendAll(elems: Traverser[Byte]): Unit = {
  ...
  override def appendArray(elems: Array[Byte]): Unit = {
    val n = elems.length
    var array = buffer
    if (aliased || size + n > array.length) {
      array = new Array[Byte](expand(size + n))
  ...
  override def prepend(elem: Byte): Unit = {
    var array = buffer
  ...
  override def prependAll(elems: Traverser[Byte]): Unit = {
    if (elems.isInstanceOf[ArrayLike[_]]) {
      val xs = elems.asInstanceOf[ArrayLike[Byte]]
      val n = xs.length
      var array = buffer
  ...
  override def prependArray(elems: Array[Byte]): Unit = {
    val n = elems.length
    var array = buffer
    if (aliased || size + n > array.length) array = new Array[Byte](expand(n + size))
    if (buffer != null) java.lang.System.arraycopy(buffer, 0, array, n, size)
  ...
  override def insert(index: Int, elem: Byte): Unit = {
  ...
  override def insertAll(index: Int, elems: Traverser[Byte]): Unit = {
  ...
  override def insertArray(index: Int, elems: Array[Byte]): Unit = {
  ...
  override def expect(count: Int): this.type = {
    if (buffer == null || size + count > buffer.length) {
      val array = new Array[Byte](size + count)
      if (buffer != null) java.lang.System.arraycopy(buffer, 0, array, 0, size)
      buffer = array
      aliased = false
    }
...
private[collections] final class ByteArrayBufferIterator private (
...
private[collections] final class ByteArrayBufferBuilder extends ByteArrayBuffer with State[ArrayBuffer[Byte]] {
  override def state: ArrayBuffer[Byte] = copy
  override def toString: String = "ArrayBuffer"+"."+"Builder"+"["+"Byte"+"]"+"()"
}
