...
private[basis] class ElseMacros(val c: blackbox.Context { type PrefixType <: ElseOps[_, _] }) {
  ...
  def recover[X : WeakTypeTag, B : WeakTypeTag](q: Expr[PartialFunction[B, X]]): Expr[X Else B] = Expr[X Else B](q"""{
    val r = $prefix.__
    val q = $q
    if (r.canSafelyTrap && q.isDefinedAt(r.trap)) _root_.basis.Bind(q.applyOrElse(r.trap, _root_.scala.PartialFunction.empty)) else r
  }""")
  ...
  def filter[A : WeakTypeTag, B : WeakTypeTag](p: Expr[A => Boolean]): Expr[A Else B] = Expr[A Else B](q"""{
    val r = $prefix.__
    if (r.canTrap || $p(r.bind)) r else _root_.basis.Trap
  }""")

  def fuse[A : WeakTypeTag, Y : WeakTypeTag](trip: Expr[Throwable => Trap[Y]]): Expr[FuseOps[A, Y]] = Expr[FuseOps[A, Y]](q"new _root_.basis.FuseOps($prefix, $trip)")
  def fuseTry[A : WeakTypeTag](isTry: Expr[_ <:< Throwable]): Expr[FuseOps[A, Throwable]]           = Expr[FuseOps[A, Throwable]](q"new _root_.basis.FuseOps($prefix, _root_.basis.Trap.NonFatal)")
  ...
  implicit protected def FuseOpsTag[A : WeakTypeTag, B : WeakTypeTag]: WeakTypeTag[FuseOps[A, B]] = WeakTypeTag(appliedType(mirror.staticClass("basis.FuseOps").toType, weakTypeOf[A] :: weakTypeOf[B] :: Nil))
