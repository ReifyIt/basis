...
final class MersenneTwister32 private (
  ...
  extends Random with Arbitrary[Int] {
  ...
  override def randomInt(): Int = apply()

  override def randomLong(): Long = (apply().toLong << 32) | apply().toLong
  ...
  override def randomBoolean(): Boolean = (apply() >>> 31) != 0
