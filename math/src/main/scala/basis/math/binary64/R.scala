...
object R {
  ...
  def apply(dim: Int): RN with AffineSpace = dim match {
    ...
    case 3 => R3
    case 4 => R4
    case _ => new R(dim).asInstanceOf[RN with AffineSpace] // cast avoids compiler crash
  }

  /** Returns a new double-precision floating-point matrix space. */
  def apply(RowSpace: RN, ColSpace: RN): RMxN { val Row: RowSpace.type; val Col: ColSpace.type } =
