...
/** An abstract double-precision floating-point matrix space.
  ...
  * @group    Real
  */
trait RMxN extends FMxN { RMxN =>
  override type Matrix <: MatrixRMxN

  override val Transpose: RMxN {
    val Row: RMxN.Col.type
    val Col: RMxN.Row.type
  }
  ...
  override def product
    ...
    if (matrixA.isInstanceOf[MatrixRMxN] && matrixB.isInstanceOf[MatrixRMxN]) {
      val realMatrixA = matrixA.asInstanceOf[MatrixRMxN]
      val realMatrixB = matrixB.asInstanceOf[MatrixRMxN]
  ...
  trait MatrixRMxN extends Any with MatrixFMxN {
    override def Row: RMxN.Row.type = RMxN.Row
    override def Col: RMxN.Col.type = RMxN.Col
