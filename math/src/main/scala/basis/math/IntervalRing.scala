...
/** A ring of closed sets between two elenents of an ordered ring.
  ...
  */
trait IntervalRing extends Ring { IntervalRing =>
  ...
  type Interval <: IntervalRingElement
  ...
  /** A closed set of elements between two points of an ordered ring.
    ...
    */
  trait IntervalRingElement extends Any with RingElement {
    ...
    override def + (that: Interval): Interval =
      IntervalRing(lower + that.lower, upper + that.upper)

    override def unary_- : Interval =
      IntervalRing(-upper, -lower)

    override def - (that: Interval): Interval =
      IntervalRing(lower - that.upper, upper - that.lower)
    ...
    override def equals(other: Any): Boolean = other match {
      case that: IntervalRingElement => lower.equals(that.lower) && upper.equals(that.upper)
      case _ => false
    }
    ...
    override def toString: String = {
      val s = new java.lang.StringBuilder(IntervalRing.toString)
      s.append('(')
      s.append(lower)
      s.append(", ")
      s.append(upper)
