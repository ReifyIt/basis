...
sealed abstract class ByteVectorLE extends ByteVector with ByteOrder[LittleEndian] {
  ...
  override def mutateFloat(address: Long, value: Float): ByteVectorLE = mutateInt(address, value.toIntBits)

  override def mutateDouble(address: Long, value: Double): ByteVectorLE = mutateLong(address, value.toLongBits)
