...
final class ByteBufferBE(val __ : Array[Byte]) extends AnyVal with ByteBuffer with ByteOrder[BigEndian] {
  ...
  override def ++ (that: Loader): ByteBufferBE = that match {
    case that: ByteBuffer =>
      ...
      new ByteBufferBE(buffer)
    case _ =>
      val framer = ByteBufferBE.Framer.expect(size + that.size)
      framer.writeData(this)
      framer.writeData(that)
      framer.state
  }

  override def reader(address: Long): Reader with ByteOrder[BigEndian] = new ByteBufferBEReader(__, address.toInt)
...
object ByteBufferBE extends ByteOrder[BigEndian] with ByteFactory[ByteBufferBE] {
  ...
  override def toString: String = "ByteBufferBE"
}

private[data] final class ByteBufferBEReader(buffer: Array[Byte], private[this] var offset: Int) extends ByteOrder[BigEndian] with Reader {
  override def endian: BigEndian = BigEndian
...
private[data] final class ByteBufferBEFramer extends State[ByteBufferBE] with ByteOrder[BigEndian] with Framer {
  ...
  override def state: ByteBufferBE = {
    if (buffer == null) buffer = ByteBufferBE.empty.__
  ...
  override def toString: String = "ByteBufferBE"+"."+"Framer"+"()"
}
