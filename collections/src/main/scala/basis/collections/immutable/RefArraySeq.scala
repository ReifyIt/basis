...
private[collections] final class RefArraySeq[+A](array: Array[AnyRef]) extends ArraySeq[A] {
  ...
  override def appendAll[B >: A](elems: Traverser[B]): ArraySeq[B] = {
  ...
  override def prependAll[B >: A](elems: Traverser[B]): ArraySeq[B] = {
    if (elems.isInstanceOf[ArrayLike[_]]) {
      val xs = elems.asInstanceOf[ArrayLike[B]]
      val n = xs.length
  ...
  override def insertAll[B >: A](index: Int, elems: Traverser[B]): ArraySeq[B] = {
    if (elems.isInstanceOf[ArrayLike[_]]) {
      val xs = elems.asInstanceOf[ArrayLike[B]]
      val n = xs.length
      if (index < 0 || index > length) throw new IndexOutOfBoundsException(index.toString)
...
private[collections] final class RefArraySeqIterator[A]
