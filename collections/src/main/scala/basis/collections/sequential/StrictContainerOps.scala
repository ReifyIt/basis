...
final class StrictContainerOps[+A, -Family](val __ : Container[A]) extends AnyVal {
  ...
  def ++ [B >: A](those: Container[B])(implicit builder: Builder[B] with From[Family]): builder.State     = macro StrictTraverserMacros.++[B]
...
private[sequential] object StrictContainerMacros {
