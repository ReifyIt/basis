...
final class StrictCollectionOps[+A, -Family](val __ : Collection[A]) extends AnyVal {
  ...
  def dropWhile(p: A => Boolean)(implicit builder: Builder[A] with From[Family]): builder.State =
    new StrictTraverserOps[A, Family](__).dropWhile(p)(builder)

  def takeWhile(p: A => Boolean)(implicit builder: Builder[A] with From[Family]): builder.State =
    new StrictTraverserOps[A, Family](__).takeWhile(p)(builder)
  ...
  def span(p: A => Boolean)
    ...
    new StrictTraverserOps[A, Family](__).span(p)(builder1, builder2)

  def drop(lower: Int)(implicit builder: Builder[A] with From[Family]): builder.State =
    new StrictTraverserOps[A, Family](__).drop(lower)(builder)

  def take(upper: Int)(implicit builder: Builder[A] with From[Family]): builder.State =
    new StrictTraverserOps[A, Family](__).take(upper)(builder)

  def slice(lower: Int, upper: Int)(implicit builder: Builder[A] with From[Family]): builder.State =
    new StrictTraverserOps[A, Family](__).slice(lower, upper)(builder)

  def ++ [B >: A](those: Collection[B])(implicit builder: Builder[B] with From[Family]): builder.State =
    new StrictTraverserOps[A, Family](__).++[B](those)(builder)
}
