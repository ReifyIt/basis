...
private[sequential] class CollectionMacros(val c: blackbox.Context) {
  ...
  implicit protected def MaybeTag[A](implicit A: WeakTypeTag[A]): WeakTypeTag[Maybe[A]] =
    WeakTypeTag(appliedType(mirror.staticPackage("basis.util").moduleClass.typeSignature.member(TypeName("Maybe")).asType.toTypeConstructor, A.tpe :: Nil))
  ...
  protected def FamilyTag(xs: Expr[Family[_]]): WeakTypeTag[xs.value.Family] =
    WeakTypeTag(typeRef(StableType(xs), mirror.staticClass("basis.collections.Family").toType.member(TypeName("Family")), Nil).dealias)
