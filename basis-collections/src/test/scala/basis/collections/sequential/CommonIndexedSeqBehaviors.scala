...
trait CommonIndexedSeqBehaviors extends CommonSeqBehaviors { this: FunSpec =>
  import common.CommonIndexedSeqOps
  import ShouldMatchers._
  
  def GenericCommonIndexedSeq[CC[A] <: IndexedSeq[A]](CC: BuilderFactory[CC]) {
    describe(s"A common indexed $CC sequence") {
      ...
      it("should select an element defined for a partial function") {
        val xs = CC(2, 4, 5, 8)
        xs.select {
          case 5 => "five"
        } should be (Some("five"))
      ...
      it("should select None when no element is defined for a partial function") {
        val xs = CC(1, 2, 3, 4)
        xs.select {
          case 0 => "zero"
        } should be (None)
