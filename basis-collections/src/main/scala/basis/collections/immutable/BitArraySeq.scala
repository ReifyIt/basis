...
private[collections] final class BitArraySeq(words: Array[Int], override val length: Int) extends ArraySeq[Boolean] {
  override def erasure: ClassTag[_] = ClassTag.Boolean
...
private[collections] final class BitArraySeqIterator
...
private[collections] final class BitArraySeqBuilder extends Builder[Boolean] with State[ArraySeq[Boolean]] {
  ...
  override def append(value: Boolean): Unit = {
    prepare(length + 1)
  ...
  override def toString: String = "ArraySeq"+"."+"Builder"+"["+"Boolean"+"]"+"()"
}
