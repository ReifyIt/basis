...
/** General container operations.
  ...
  */
abstract class GeneralContainerOps[A, From] private[sequential] {
  ...
  def eagerly: StrictContainerOps[A, From] =
    macro GeneralContainerOps.eagerly[A, From]
...
private[sequential] object GeneralContainerOps {
  ...
  def choose[A : c.WeakTypeTag, B : c.WeakTypeTag]
    ...
    : c.Expr[Option[B]] =
    new IteratorMacros[c.type](c).choose[A, B](unApplyIterator(c))(q)
  ...
  private def ContainerTag[A : c.WeakTypeTag](c: Context): c.WeakTypeTag[Container[A]] = {
    ...
    c.WeakTypeTag(
      appliedType(
        c.mirror.staticClass("basis.collections.traversable.Container").toType,
        weakTypeOf[A] :: Nil))
  }
