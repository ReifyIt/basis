...
/** General enumerator operations.
  ...
  */
class GeneralEnumeratorOps[A, From](val __ : Enumerator[A]) extends AnyVal {
  ...
  def reduceLeft[B >: A](op: (B, A) => B): B = {
    val f = new GeneralEnumeratorOps.ReduceLeft(op.asInstanceOf[(B, B) => B]) // FIXME: waiting on SI-6482
    traverse(__)(f)
    if (f.isDefined) f.state
    else throw new java.lang.UnsupportedOperationException
  }
