...
/** General indexed sequence operations.
  ...
  */
abstract class GeneralIndexedSeqOps[A, From] private[sequential] {
  ...
  def reduceOption[B >: A](op: (B, B) => B): Option[B] =
    macro GeneralIndexedSeqOps.reduceLeftOption[A, B]
  ...
  def reduceRightOption[B >: A](op: (A, B) => B): Option[B] =
    macro GeneralIndexedSeqOps.reduceRightOption[A, B]
  ...
  def eagerly: StrictIndexedSeqOps[A, From] =
    macro GeneralIndexedSeqOps.eagerly[A, From]
...
private[sequential] object GeneralIndexedSeqOps {
  ...
  def choose[A : c.WeakTypeTag, B : c.WeakTypeTag]
    ...
    : c.Expr[Option[B]] =
    new IndexedSeqMacros[c.type](c).choose[A, B](unApply(c))(q)
  
  def eagerly[A : c.WeakTypeTag, From : c.WeakTypeTag](c: Context): c.Expr[StrictIndexedSeqOps[A, From]] = {
    import c.universe._
    c.Expr {
      Apply(
    ...
    } (StrictIndexedSeqOpsTag[A, From](c))
  }
  ...
  private def IndexedSeqTag[A : c.WeakTypeTag](c: Context): c.WeakTypeTag[IndexedSeq[A]] = {
    ...
    c.WeakTypeTag(
      appliedType(
        c.mirror.staticClass("basis.collections.traversable.IndexedSeq").toType,
        weakTypeOf[A] :: Nil))
  }
  
  private def StrictIndexedSeqOpsTag[A : c.WeakTypeTag, From : c.WeakTypeTag](c: Context)
    : c.WeakTypeTag[StrictIndexedSeqOps[A, From]] = {
    import c.universe._
    c.WeakTypeTag(
      appliedType(
