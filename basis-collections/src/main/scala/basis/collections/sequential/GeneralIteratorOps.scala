...
/** General iterator operations.
  ...
  */
abstract class GeneralIteratorOps[A, From] private[sequential] {
  ...
  def eagerly: StrictIteratorOps[A, From] =
    macro GeneralIteratorOps.eagerly[A, From]
...
private[sequential] object GeneralIteratorOps {
  ...
  def choose[A : c.WeakTypeTag, B : c.WeakTypeTag]
    ...
    : c.Expr[Option[B]] =
    new IteratorMacros[c.type](c).choose[A, B](unApply(c))(q)
  
  def eagerly[A : c.WeakTypeTag, From : c.WeakTypeTag](c: Context): c.Expr[StrictIteratorOps[A, From]] = {
    import c.universe._
    c.Expr {
      Apply(
  ...
  private def StrictIteratorOpsTag[A : c.WeakTypeTag, From : c.WeakTypeTag](c: Context)
    : c.WeakTypeTag[StrictIteratorOps[A, From]] = {
    import c.universe._
    c.WeakTypeTag(
      appliedType(
