...
/** General linear sequence operations.
  ...
  */
abstract class GeneralLinearSeqOps[A, From] private[sequential] {
  ...
  def reduceOption[B >: A](op: (B, B) => B): Option[B] =
    macro GeneralLinearSeqOps.reduceLeftOption[A, B]
  ...
  def eagerly: StrictLinearSeqOps[A, From] =
    macro GeneralLinearSeqOps.eagerly[A, From]
...
private[sequential] object GeneralLinearSeqOps {
  ...
  def choose[A : c.WeakTypeTag, B : c.WeakTypeTag]
    ...
    : c.Expr[Option[B]] =
    new LinearSeqMacros[c.type](c).choose[A, B](unApply(c))(q)
  
  def eagerly[A : c.WeakTypeTag, From : c.WeakTypeTag](c: Context): c.Expr[StrictLinearSeqOps[A, From]] = {
    import c.universe._
    c.Expr {
      Apply(
    ...
    } (StrictLinearSeqOpsTag[A, From](c))
  }
  ...
  private def LinearSeqTag[A : c.WeakTypeTag](c: Context): c.WeakTypeTag[LinearSeq[A]] = {
    ...
    c.WeakTypeTag(
      appliedType(
        c.mirror.staticClass("basis.collections.traversable.LinearSeq").toType,
        weakTypeOf[A] :: Nil))
  }
  
  private def StrictLinearSeqOpsTag[A : c.WeakTypeTag, From : c.WeakTypeTag](c: Context)
    : c.WeakTypeTag[StrictLinearSeqOps[A, From]] = {
    import c.universe._
    c.WeakTypeTag(
      appliedType(
