...
private[containers] class RefArrayBuffer[A] private (
  ...
  private[containers] final def toArraySeq: ArraySeq[A] = {
...
private[containers] final class RefArrayBufferIterator[+A] private (
    private[this] val xs: RefArrayBuffer[A],
    private[this] var i: Int,
    private[this] var n: Int,
    private[this] var elem: A)
  extends Iterator[A] {
  
  def this(xs: RefArrayBuffer[A]) = this(xs, 0, xs.length, if (!xs.isEmpty) xs(0) else null.asInstanceOf[A])
  ...
  override def step() {
    ...
    i += 1
    n = xs.length
    elem = if (i < n) xs(i) else null.asInstanceOf[A]
  }
  
  override def dup: Iterator[A] = new RefArrayBufferIterator(xs, i, n, elem)
}
