...
trait DataSource[+DataType] {
  def empty: DataType = Framer().state

  def apply(data: Array[Byte]): DataType = {
    var i = 0
    val n = data.length
    val framer = Framer().expect(n.toLong)
    while (i < n) {
  ...
  def apply(base64: CharSequence): DataType = {
    val framer = Framer()
    val n = base64.length
    var i = 0
  ...
  implicit def Framer(): Framer with State[DataType]
}
