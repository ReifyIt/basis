...
private[util] class TruthMacros(val c: blackbox.Context { type PrefixType <: TruthOps }) {
  import c.{ Expr, prefix }
  import c.universe._

  def isTrue: Expr[Boolean]  = Expr[Boolean](q"_root_.basis.util.True eq $prefix.__")
  def isFalse: Expr[Boolean] = Expr[Boolean](q"_root_.basis.util.False eq $prefix.__")
}
